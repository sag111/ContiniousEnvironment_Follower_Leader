...
class ContinuousObserveModifier_v0(ObservationWrapper):
    ...
    def __init__(self, env, action_values_range=None, lz4_compress=False):
        ...
        if 'LeaderCorridor_Prev_lasers_v2_compas' in self.follower_sensors:
            if 'front_lasers_count' in env.follower_sensors['LeaderCorridor_Prev_lasers_v2_compas']:
                features_number += (
                            4 * env.follower_sensors['LeaderCorridor_Prev_lasers_v2_compas']['front_lasers_count'])
            if 'back_lasers_count' in env.follower_sensors['LeaderCorridor_Prev_lasers_v2_compas']:
                features_number += (
                            4 * env.follower_sensors['LeaderCorridor_Prev_lasers_v2_compas']['back_lasers_count'])
        ...
        if 'LaserSensor' in self.follower_sensors:
            if self.follower_sensors['LaserSensor']['return_all_points']:
                lidar_points_number = (int(
                    self.follower_sensors['LaserSensor']['available_angle'] / self.follower_sensors['LaserSensor'][
                        'angle_step']) + 1) * self.follower_sensors['LaserSensor']['points_number']
            else:
                lidar_points_number = (int(
                    self.follower_sensors['LaserSensor']['available_angle'] / self.follower_sensors['LaserSensor'][
                        'angle_step']) + 1)
    ...
    def observation(self, obs):
        ...
        if 'LeaderCorridor_lasers' in self.follower.sensors:
            ...
            corridor_lasers = np.clip(corridor_lasers / self.follower.sensors['LeaderCorridor_lasers'].laser_length, 0,
                                      1)
        ...
        if 'LeaderCorridor_lasers_v2' in self.follower.sensors:
            ...
            corridor_lasers_v2 = np.clip(
                corridor_lasers_v2 / self.follower.sensors['LeaderCorridor_lasers_v2'].laser_length, 0, 1)
        ...
        if 'LeaderObstacles_lasers' in self.follower.sensors:
            ...
            corridor_obs_lasers = np.clip(
                corridor_obs_lasers / self.follower.sensors['LeaderObstacles_lasers'].laser_length, 0, 1)
        ...
        if 'Leader_Dyn_Obstacles_lasers' in self.follower.sensors:
            ...
            corridor_obs_lasers = np.clip(
                corridor_obs_lasers / self.follower.sensors['Leader_Dyn_Obstacles_lasers'].laser_length, 0, 1)
            features_list.append(corridor_obs_lasers)
        ...
        if 'LeaderCorridor_Prev_lasers_v2' in self.follower.sensors:
            ...
            corridor_prev_lasers_v2 = np.clip(
                corridor_prev_lasers_v2 / self.follower.sensors['LeaderCorridor_Prev_lasers_v2'].laser_length, 0, 1)
            # features_list.append(corridor_lasers_v2)

        if 'LaserPrevSensor' in self.follower.sensors:
            corridor_prev_obs_lasers = obs['LaserPrevSensor']
            corridor_prev_obs_lasers = np.clip(
                corridor_prev_obs_lasers / self.follower.sensors['LaserPrevSensor'].laser_length, 0, 1)
            # features_list.append(corridor_obs_lasers)
        ...
        if 'LeaderCorridor_Prev_lasers_v2_compas' in self.follower.sensors:
            ...
            corridor_prev_lasers_v2 = np.clip(
                corridor_prev_lasers_v2 / self.follower.sensors['LeaderCorridor_Prev_lasers_v2_compas'].laser_length, 0,
                1)

        if 'LaserPrevSensor_compas' in self.follower.sensors:
            corridor_prev_obs_lasers = obs['LaserPrevSensor_compas']
            corridor_prev_obs_lasers = np.clip(
                corridor_prev_obs_lasers / self.follower.sensors['LaserPrevSensor_compas'].laser_length, 0, 1)
        ...
        if 'LaserSensor' in self.follower_sensors:
            ...
            lidar_sensed_points = np.clip(
                lidar_sensed_points / (self.follower.sensors["LaserSensor"].range * self.PIXELS_TO_METER), -1, 1)
...
class ContinuousObserveModifierPrev(ObservationWrapper):
    ...
    def observation(self, obs):
        ...
        if 'LeaderCorridor_Prev_lasers_v2' in self.follower.sensors:
            ...
            corridor_lasers_v2 = np.clip(
                corridor_lasers_v2 / self.follower.sensors['LeaderCorridor_Prev_lasers_v2'].laser_length, 0, 1)
            # features_list.append(corridor_lasers_v2)

        if 'LaserPrevSensor' in self.follower.sensors:
            corridor_obs_lasers = obs['LaserPrevSensor']
            corridor_obs_lasers = np.clip(corridor_obs_lasers / self.follower.sensors['LaserPrevSensor'].laser_length,
                                          0, 1)
            # features_list.append(corridor_obs_lasers)
...
class ContinuousObserveModifier_lidarMap2d(ContinuousObserveModifier_v0):
    def __init__(self,
        ...
        self.lidar_angle_steps_count = 1 + self.follower_sensors['LaserSensor']['available_angle'] // \
                                       self.follower_sensors['LaserSensor']['angle_step']
    ...
    def DrawLidar2dMap(self, lidar_map_size,
        ...
        if self.add_safezone_on_map:
            ...
            relative_to_follower_rectangle_points_rotated = [rotateVector(x, -follower_direction) for x in
                                                             relative_to_follower_rectangle_points]
            ...
            for i in range(lidar_map.shape[0]):
                for j in range(lidar_map.shape[1] - 1):
                    if lidar_map[i, j, 2] == 1:
                        if i == 0:
                            if j == 0:
                                if lidar_map[i, j + 1, 2] == 1 and lidar_map[i + 1, j, 2] == 1 and lidar_map[
                                    -1, j, 2] == 1:
                                    continue
                            if lidar_map[i, j + 1, 2] == 1 and lidar_map[i + 1, j, 2] == 1 and lidar_map[
                                i, j - 1, 2] == 1 and lidar_map[-1, j, 2] == 1:
                                continue
                        elif i == lidar_map.shape[0] - 1:
                            if j == 0:
                                if lidar_map[i, j + 1, 2] == 1 and lidar_map[0, j, 2] == 1 and lidar_map[
                                    i - 1, j, 2] == 1:
                                    continue
                            if lidar_map[i, j + 1, 2] == 1 and lidar_map[0, j, 2] == 1 and lidar_map[
                                i, j - 1, 2] == 1 and lidar_map[i - 1, j, 2] == 1:
                                continue
                        elif j == 0:
                            if lidar_map[i, j + 1, 2] == 1 and lidar_map[i + 1, j, 2] == 1 and lidar_map[
                                i - 1, j, 2] == 1:
                                continue
                        elif lidar_map[i, j + 1, 2] == 1 and lidar_map[i + 1, j, 2] == 1 and lidar_map[
                            i, j - 1, 2] == 1 and lidar_map[i - 1, j, 2] == 1:
                            continue
                        else:
    ...
    def observation(self, obs):
        ...
        arccos_x = np.arccos(relative_leader_position_2.dot(np.array([1, 0])) / (
                    np.linalg.norm(relative_leader_position_2) * np.linalg.norm(np.array([1, 0]))))
        arccos_y = np.arccos(relative_leader_position_2.dot(np.array([0, 1])) / (
                    np.linalg.norm(relative_leader_position_2) * np.linalg.norm(np.array([0, 1]))))
        if arccos_y > np.pi / 2:
            arccos_x = -arccos_x
        angle_between_leader_and_follower = np.degrees(arccos_x)

        lidar_map = self.DrawLidar2dMap((self.lidar_angle_steps_count, self.lidar_points_number), leader_position,
                                        follower_position,
                                        angle_between_leader_and_follower, obs["LaserSensor"],
                                        self.follower_sensors['LaserSensor']['angle_step'],
...
class ContinuousObserveModifier_lidarMap2d_v2(ContinuousObserveModifier_lidarMap2d):
            ...
            if len(self.corridor) > 2:
                for i in range(len(self.corridor) - 1):
                    ...
                    if ((check1 == [False, True]).all() and (check2 == [False, True]).all()):
                        ...
                        raise ValueError(
                            "Не предвидел такой вариант расположения вершин прямоугольника (сегмента корридора) при проверке, находятся ли точки внутри него: check1:{}, check2:{}".format(
    ...
    def constructCorridor(self, relative_leader_position, follower_position, follower_direction):
        if self.saving_counter % self.saving_period == 0:
            ...
            if len(self.leader_positions_hist) > 0 and (
                    self.leader_positions_hist[-1] == relative_leader_position).all():
                return
